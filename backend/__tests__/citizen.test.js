...
describe('Citizen API', () => {
    ...
    describe('POST /api/citizen', () => {
        ...
        test('/api/citizen [correct input]', async () => {
            const data = { firstname: 'John', lastname: 'Doe', birthdate: '2000-01-01', gender: 'm', street: 'Musterweg', housenumber: '34b', city_code: 12345, city: 'Musterhausen' };
        ...
        test('/api/citizen [wrong input]', async () => {
            const data = { firstname: 'John', lastname: 'Doe', birthdate: '2000-01-01' };
            createCitizen.mockResolvedValue(1);
            const response = await request(app).post('/api/citizen').send(data);
            //response is correct
            expect(response.statusCode).toBe(400);
