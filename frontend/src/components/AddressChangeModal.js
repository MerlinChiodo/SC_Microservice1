...
export const AddressChangeModal = ({ context, id, innerProps }) => (
    <form onSubmit={(event) => innerProps.handleSubmit(event, id)}>
        <Grid grow gutter="xl" align="center">
            ...
            <Grid.Col span={12} xs={4}>
                <NumberInput label="Postleitzahl" name="city_ode" placeholder="Bitte eingeben" hideControls required />
            </Grid.Col>
            <Grid.Col span={12} xs={8}>
                <TextInput label="Ort" name="city" placeholder="Bitte eingeben" required />
            </Grid.Col>
            <Grid.Col span={12}>
                <Textarea label="Begründung" name="reasoning" minRows={7} placeholder="Bitte beschreibe warum die Adressänderung beantragt wird" />
            </Grid.Col>
